...
const CreateVisit = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            await adminAxios.post('/technical-visits', formattedData);
            navigate(-1);
        } catch (err) {
            console.error('Error creating technical visit:', err);

            if (err.response && err.response.data && err.response.data.errors) {
                setFormErrors(err.response.data.errors);
            } else {
                setError('Failed to create technical visit. Please try again.');
            }
        } finally {
            setLoading(false);
        }
    ...
    return (
        <div className="container mx-auto p-4">
            <div className="max-w-2xl mx-auto bg-white rounded-lg shadow p-6">
                <h1 className="text-2xl font-bold mb-6">Nouvelle Visite Technique</h1>
                ...
                <form onSubmit={handleSubmit}>
                    <div className="mb-4">
                        ...
                        {id ?
                            <p>{currentCar.marque} - {currentCar.modele} - {currentCar.immatriculation}</p>
                            :
                            <select
                                id="car_id"
                                name="car_id"
